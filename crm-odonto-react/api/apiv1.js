...
export default async function handler(req, res) {
  ...
  // Segmentos após /api/v1/. O rewrite do vercel.json injeta ?p=<caminho>;
  // caímos pra req.query.path (catch-all) e por fim pra req.url por segurança.
  let path = [];
  if (typeof req.query.p === 'string') path = req.query.p.split('/').filter(Boolean);
  if (!path.length) path = [].concat(req.query.path || []).filter(Boolean);
  if (!path.length) {
    const semQuery = (req.url || '').split('?')[0];
    path = semQuery.replace(/^\/api\/(v1|apiv1)\/?/, '').split('/').filter(Boolean);
  }
